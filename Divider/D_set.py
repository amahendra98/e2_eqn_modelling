...
class D_set(torch.utils.data.Dataset):
    def __init__(self, res, r_min, r_max, u_min, u_max, rand):
        self.res = res
        if rand:
            self.r = np.random.uniform(r_min, r_max, size=(res*res,1))
            self.u = np.random.uniform(u_min, u_max, size=(res*res,1))
        else:
    ...
    def __getitem__(self,idx):
        ...
        v = (r_row/u_row)
        inp = np.vstack((r_row, u_row))

        sample = {'x_data': inp,'y_data': v}
